...
func main() {
	...
	if gamesCodesURL == "" {
		log.Fatalf("GAMECODES_URL environment variable is empty. Set it to scrap codes from a URL")
	}
	redemptionServiceURL := os.Getenv("REDEMPTION_SERVICE_URL")
	if redemptionServiceURL == "" {
		log.Fatalf("REDEMPTION_SERVICE_URL environment variable is empty. Set it to the backend's URL")
	}
...
func ExtractCodesFromBody(body io.Reader, codes map[string]struct{}) error {
	...
	fmt.Printf("The following codes were found: %v", codes)
	return nil
}
...
func SendCurrentCoupons(url string, extractedCoupons map[string]struct{}) error {
	...
	if resp.StatusCode != http.StatusCreated {
		return fmt.Errorf("post gamecodes did not result in 200 http code, got code: %d", resp.StatusCode)
	}
	return nil
}
