...
func (cfg *apiConfig) HandleNewCoupon(w http.ResponseWriter, r *http.Request) {
	...
	if len(params.Codes) == 0 {
		...
		respondWithMessage(w, http.StatusBadRequest, "no codes provided in request, check documentation for proper request format.")
		return
	}
...
func (cfg *apiConfig) HandleGetCoupons(w http.ResponseWriter, r *http.Request) {
	...
	if couponCode == "" {
		...
		if err != nil {
			respondWithMessage(w, http.StatusInternalServerError, "Couldn't retrieve coupons")
			return
		}
		...
		if err != nil {
			respondWithMessage(w, http.StatusInternalServerError, "Couldn't retrieve coupon")
			return
		}
